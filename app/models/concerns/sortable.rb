...
# Allow asscociations on a resource to be sorted arbitrarily.
module Sortable
  ...
  module ClassMethods
    ...
    def can_sort(collection, options = {})
      ...
      options[:join_model] ||= reflections[collection].options[:through]
      options[:attribute] ||= :position

      # create method for accessing the order collection
      define_method "ordered_#{collection}" do |*_arguments|
  ...
  def sort(collection, ids, options = {})
    ...
    options[:join_model] ||= self.class.reflections[collection]
                             .options[:through]
    options[:attribute] ||= :position
