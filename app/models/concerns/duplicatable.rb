...
#   end
#
# Now `alice.hobbies` will include `bob.hobbies` if you set
# `bob.duplicate_of alice`.
#
# = Setup
#
# You need to add a migration to your model in order to use
# this concern:
#   rails g migration AddArchetypeToMODEL \
#         archetype:references{polymorphic}
#   rake db:migrate
#
module Duplicatable
  ...
  included do
    has_many :duplicates, as: :archetype, class_name: name
  ...
  def duplicate_of(resource)
    if self.class != resource.class
      raise TypeError, "Cannot mark a #{resource.class.name} "\
                       "as duplicate of a #{self.class.name}"
    end
  ...
  def duplicate_candidates
    ...
    search.results.reject { |x| x.id == id }
  end
  ...
  module ClassMethods
    ...
    # `assembly.parts` will include `Part`s from both `assembly` and
    # `assembly_dup`.
    #
    ...
    def include_associations_of_dups(*associations)
      ...
      associations = [{ only: associations }] unless associations[0].is_a? Hash
      ...
      associations.each do |name|
        ...
        define_method name do |*arguments|
          return super(arguments) if duplicates.empty? || @dedup

          w = [] # where clause
          type, key, tbl = get_sql_names_for_combining name

          # do we need to specify the resource type?
          # (if polymorphic, for example)
          w << "#{tbl}.#{type}='#{self.class.name}'" if type.present?

          # get the IDs of each duplicate, and self
          ids = [id] + duplicates.map(&:id)
          ids.map! { |i| "#{tbl}.#{key}=#{i}" }
          w << "(#{ids.join(' or ')})" unless ids.empty?

          wsql = w.join ' and '

          # construct relation
          r = association(name).reader(arguments).unscope(where: key)
                               .where(wsql)
    ...
    def top_duplicate_candidates(limit = 10)
      ...
      list = all.map { |x| [x, x.duplicate_candidates.length] }
    ...
    def validate_associations_for_combining(associations)
      associations.each do |name|
        unless reflections.key? name
          raise ArgumentError, "No such association: #{name}"
        end
        next if valid_association_for_combining?(name)
        raise ArgumentError, 'Association type not allowed: ' +
                             reflection[name.to_s].macro
      end
    end
    ...
    def build_associations_from_hash(association_hash)
      if association_hash.key? :only
        ...
        raise ArgumentError, 'Argument hash contains neither :only nor '\
                             ':except key'
      end
    end
  end
  ...
  # Get the table name, foreign key and foreign type used when joining
  # associations.
  #
  ...
  def get_sql_names_for_combining(name, options = {})
    ...
    if reflection.macro == :has_many && reflection.options.key?(:through)
