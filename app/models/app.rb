...
class App < ActiveRecord::Base
  ...
  validates :website,
            format: %r{\Ahttps?://.*}i
  validates :support_url, :callback_url, :author_url,
            allow_blank: true,
            format: %r{\Ahttps?://.*}i
  ...
  def self.verify_request(request, options = {}, &block)
    ...
    return false unless OauthNonce.remember(signature.request.nonce,
  ...
  def self.permissions
    %w(name picture playlists listens shares)
  end
  ...
  def create_request_token(_params = {})
    RequestToken.create app: self, callback_url: token_callback_url
  end
  ...
  def image(size = :huge)
    ...
    m = "icon_#{sizes[size]}"
    return send(m) if respond_to?(m) && send(m).present?
