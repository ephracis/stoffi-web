  ...
  class Song < ActiveRecord::Base
    ...
    with_options uniq: true do |assoc|
      ...
      assoc.has_and_belongs_to_many :artists do
        def ==(other)
          other = other.join(',') if other.is_a? Array
          Artist.uniq_name(join(',')) == Artist.uniq_name(other)
        end
      end
    end
    ...
    def similar(count = 5)
      ...
      r = []
      s.each do |x|
        r.concat(x.songs.where.not(id: id).offset(rand(x.songs.count - 5))
                        .limit(count).to_a)
      end
    ...
    def self.find_by_hash(hash)
      ...
      where('lower(title) = ?', hash[:title].downcase).find_each do |song|
        return song if artist_txt.blank? ||
                       join_artists(song.artists) == artist_txt
      end
      nil
    end
