...
module Media
  ...
  class Album < ActiveRecord::Base
    ...
    # The sum of the normalised popularity of the album itself, and the
    # normalised popularity of all its songs.
    def popularity
      p = super
      if songs.count > 0 && p == 0
        p += songs.inject(p) do |sum, x|
          sum + x.popularity
        end
      end
      p
    ...
    def artist_names
      artists.collect(&:name).to_sentence
    end
    ...
    def self.find_by_hash(hash)
      ...
      where('lower(title) = ?', hash[:title].downcase).find_each do |album|
    ...
    # Paginates the songs of the album. Should be called before
    # <tt>paginated_songs</tt> is called.
    #
    #   album.paginate_songs(10, 30)
    #   songs = album.paginated_songs # songs will now hold the songs 30-39
    ...
    # Returns a slice of the album's songs which was created by
