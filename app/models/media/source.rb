...
module Media
  ...
  class Source < ActiveRecord::Base
    ...
    def path
      ...
      else "stoffi:#{resource_type.demodulize.parameterize}"\
           ":#{name}:#{foreign_id}"
      end
    end
    ...
    #
    # For example, the resource with the highest popularity in the scope of the
    # source type gets 1.0.
    #
    # This makes it possible to compare the popularity between source types. The
    # most popular song on YouTube will thus compare equal to the most popular
    # song on Jamendo.
    def normalized_popularity
      max = Source.where(name: name, resource_type: resource_type)
                  .maximum(:popularity) || 1
    ...
    def self.find_by_hash(hash)
      keys = [:name]
      keys << if hash.key? :foreign_url
                :foreign_url
              else
                :foreign_id
              end
      find_by hash.select { |k, _v| k.to_sym.in?(keys) }
    end
    ...
    def self.parse_url_path(path)
      ...
      src = :url if url_path?(path)

      # if no extension, assume song
      if ext.in?(SONG_EXT) || (ext.empty? && src == :url)
        resource = 'Media::Song'
    ...
    SONG_EXT = ['.aac', '.ac3', '.aif', '.aiff', '.ape', '.apl', '.bwf',
                '.flac', '.m1a', '.m2a', '.m4a', '.mov', '.mp+', '.mp1', '.mp2',
                '.mp3', '.mp3pro', '.mp4', '.mpa', '.mpc', '.mpeg', '.mpg',
                '.mpp', '.mus', '.ofr', '.ofs', '.ogg', '.spx', '.tta', '.wav',
                '.wv'].freeze

    # A list of all supported file extensions that identifies a playlist.
    PLAYLIST_EXT = ['.b4s', '.m3u', '.pls', '.ram', '.wpl', '.asx', '.wax',
                    '.wvx', '.m3u8', '.xspf', '.xml'].freeze
  end
end
