...
module GeoipHelper
  ...
  def flag_image_tag(ip, options = {})
    ...
    code.blank? or code == '--' ? '' : image_tag("flags/#{code.downcase}.png",
      options)
  end
  ...
  def city(ip)
    city = origin_city(ip)
    city = city.present? ? city.city_name : t('cities.unknown')
  end
  ...
  def country(ip)
    country = origin_country(ip)
    code = country.present? ? country.country_code2.to_s : ''
    t((code.blank? or code == '--') ? 'countries.unknown' : "countries.#{code}")
  end
