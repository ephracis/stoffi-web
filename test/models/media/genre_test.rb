...
class GenreTest < ActiveSupport::TestCase
  ...
  test 'should get top genres' do
    ...
    assert g[0].listens.count >= g[1].listens.count,
           'Top genres not in order (first and second)'
    assert g[1].listens.count >= g[2].listens.count,
           'Top genres not in order (second and third)'
  end
  ...
  test 'should create genre' do
    ...
    assert g.images.where(url: @hash[:images][0][:url]).any?,
           "Didn't set images"
