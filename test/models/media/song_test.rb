...
class Media::SongTest < ActiveSupport::TestCase
  ...
  test "should create song" do
    assert_difference('Media::Song.count', 1, "Didn't create song") do
      p = Media::Song.create(title: 'a random title')
    end
  end
  ...
  test "should get song from search result" do
    ...
    assert_equal hit[:artists].length, s.artists.count, "Didn't assign any artist to song"
    assert_equal hit[:artists][0][:name], s.artists[0].name, "Didn't assign song to correct artist"
    assert_equal hit[:source][:popularity], s.sources[0].popularity, "Didn't assign correct popularity"
    assert_equal hit[:source][:length], s.sources[0].length, "Didn't assign correct length"
  end
  ...
  test 'should parse title with multiple artists' do
    [
      ...
      artists, title = Media::Song.parse_title(t)
      assert_equal 3, artists.length, "Didn't parse out multiple artists in: #{t}"
