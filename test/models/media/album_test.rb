...
class AlbumTest < ActiveSupport::TestCase
  ...
  setup do
    ...
    stub_request(:get, 'http://foo.com/img1.jpg')
      .to_return(body: File.new(path), status: 200)
    stub_request(:get, 'http://foo.com/img2.jpg')
      .to_return(body: File.new(path), status: 200)
  ...
  test 'should create album' do
    ...
    assert_equal @hash[:source][:popularity].to_f, s.popularity,
                 "Didn't set source popularity"
    assert_equal @hash[:source][:foreign_id], s.foreign_id,
                 "Didn't set source id"
    assert_equal @hash[:source][:foreign_url], s.foreign_url,
                 "Didn't set source url"
    assert_equal @hash[:source][:name], s.name, "Didn't set source name"
  end

  test 'should not create album without title' do
    assert_no_difference 'Media::Album.count',
                         'Created new album without title' do
      Media::Album.create
    end
  end
  ...
  test 'should find album' do
    ...
    assert_equal artists_before, album.artists.count,
                 'Changed number of artists'
  ...
  test 'should add songs' do
    ...
    album = nil
    assert_difference 'Media::Artist.count', 1,
                      "Didn't create exactly one artist" do
      assert_difference 'Media::Song.count', 1,
                        "Didn't create exactly one song" do
        album = Media::Album.find_or_create_by_hash(@hash)
      end
    end
  ...
  test 'should get top albums' do
    ...
    assert a[0].listens.count >= a[1].listens.count,
           'Top albums not in order (first and second)'
    assert a[1].listens.count >= a[2].listens.count,
           'Top albums not in order (second and third, listens)'
  end
  ...
  test 'should get popularity' do
    ...
    assert_equal p, a.popularity,
                 "Popularity isn't the combined popularity of the songs"
    ...
    assert_equal p, a.popularity,
                 "Popularity isn't the combined popularity of the sources"
  end
