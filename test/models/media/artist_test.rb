...
class ArtistTest < ActiveSupport::TestCase
  ...
  test 'should create from hash' do
    ...
    assert_equal @hash[:images][1][:url], a.images.get_size(:small).url,
                 "Didn't set image url"
    assert_equal 1, a.sources.count, "Didn't set source"
    assert_equal @hash[:source][:popularity], a.sources[0].popularity,
                 "Didn't set popularity"
    assert_equal @hash[:source][:name].to_s, a.sources[0].name,
                 "Didn't set source name"
  end

  test 'should split name' do
    a = Media::Artist.split_name('Foo &  Bar')
    assert_equal %w(Foo Bar).sort, a.sort
    ...
    assert_equal %w(Foo Bar Baz).sort, a.sort
  ...
  test 'should get top artists' do
    ...
    assert a[0].listens.count >= a[1].listens.count,
           'Top artists not in order (first and second)'
  end
end
