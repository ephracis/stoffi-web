...
class Media::PlaylistTest < ActiveSupport::TestCase
  ...
  test "should not create two playlists with same name" do
    assert_no_difference('Media::Playlist.count', "Created two playlists with same name") do
      users(:alice).playlists.create(:name => "Foo")
    end
  end

  test "playlist name uniqness should be scoped" do
    assert_difference('Media::Playlist.count', 1, "Didn't allow two users to have playlists with same name") do
