...
class LastfmTest < ActiveSupport::TestCase
  ...
  test 'search for artists' do
    stub = stub_lastfm_search 'artist', [
      { name: 'Test Artist', listeners: 123,
    ...
    assert_equal 'http://test.com/test.jpg', hits[0][:images][0][:url],
  ...
  test 'search for songs' do
    stub = stub_lastfm_search 'track', [
      { name: 'Test Track', listeners: 123, artist: 'Test Artist',
        image: [{ '#text' => 'http://test.com/test.jpg' }] },
      {}, {}, {}, {}
    ]
  ...
  test 'should start listen' do
    ...
    params[:api_sig] = Digest::MD5.hexdigest(
      params.stringify_keys.sort.flatten.join + secret
    )
    params[:format] = 'json'
  ...
  test 'should end listen' do
    ...
    stub_request(:post, url).with(body: params).to_return(
      body: { 'a' => 'b' }.to_json
    )
