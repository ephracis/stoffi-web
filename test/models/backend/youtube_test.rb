...
class YoutubeTest < ActiveSupport::TestCase
  ...
  test 'should get songs' do
    stub_youtube_videos [
      { id: 'id1', statistics: { viewCount: 20882668 }, contentDetails: { duration: 'PT7M9S' } },
      { id: 'id2', statistics: { viewCount: 60107360 }, contentDetails: { duration: 'PT7M20S' } }
    ]
      
    songs = Backends::Youtube.get_songs(['id1', 'id2'])
    
    assert_equal 2, songs.count, "Didn't get the correct number of songs"
    assert_equal 20882668, songs[0][:source][:popularity], "Didn't get the correct popularity for id1"
    assert_equal 60107360, songs[1][:source][:popularity], "Didn't get the correct popularity for id2"
    assert_equal 7*60+9,  songs[0][:source][:length], "Didn't get the correct length for id1"
    assert_equal 7*60+20, songs[1][:source][:length], "Didn't get the correct length for id2"
  end
  ...
  test 'should search for songs' do
    ...
    assert_equal 3, songs.count, "Didn't return the correct number of search hits"
