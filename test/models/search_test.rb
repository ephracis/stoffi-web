...
class SearchTest < ActiveSupport::TestCase
  ...
  test "should get suggestions" do
    ...
    assert_equal "bob marley", s[0][:value].downcase, "first suggestion wasn't correct"
  end
  
  test "should prioritize user's suggestions" do
    s = Search.suggest('bob', '/home', 50, 50, 'us', [], users(:alice).id)
    assert_equal 5, s.length, "wrong number of suggestions returned"
    assert_equal "Bob Sinclair", s[0][:value], "first suggestion wasn't correct"
  end
  
  test "should prioritize nearby suggestions" do
    s = Search.suggest('bob', '/home', 30, 30, 'us')
    assert_equal 5, s.length, "wrong number of suggestions returned"
    assert_equal "Bob Dylan", s[0][:value], "first suggestion wasn't correct"
  end
  
  test "should prioritize similar locale suggestions" do
    s = Search.suggest('bob', '/home', 50, 50, 'se')
    assert_equal 5, s.length, "wrong number of suggestions returned"
    assert_equal "Bob Andersson", s[0][:value], "first suggestion wasn't correct"
  end
  ...
  test "should parse search results from backends" do
    stub = stub_lastfm_search 'track', [
      { name: 'Test Lastfm Song', listeners: 123, artist: 'Test Artist', image: [ { '#text' => 'http://test.com/test.jpg' }]},
      {}, {}, {}, {}
    ]
    ...
    stub_youtube_videos [
      ...
      { id: 'id3', snippet: { title: 'Eminem - Test Song 4 feat. Test Artist' }}
    ]
