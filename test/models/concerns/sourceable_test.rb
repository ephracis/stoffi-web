...
class SourceableTest < ActiveSupport::TestCase
  ...
  test 'should create has_and_belongs_to_many association' do
    ...
    assert_difference 'Media::Song.count', 1, "Didn't create song" do
      ...
      assert_difference 'Media::Artist.count', 1,
                        "Didn't create single artist" do
        s = Media::Song.create_by_hash(hash)
        assert s, "Didn't return song"
        assert_equal 2, s.artists.length, "Didn't assign two artists"
        assert_includes s.artists.map(&:to_s),
                        'Eminem', "Didn't assign artist Eminem"
        assert_includes s.artists.map(&:to_s), 'Dido',
                        "Didn't assign artist Dido"
      end
    end
  end
  ...
  test 'should create associations recursively' do
    ...
    assert_difference 'Media::Event.count', 1, "Didn't create event" do
      assert_no_difference 'Media::Artist.count',
                           'Created artist although it already exists' do
        assert_difference 'media_artists(:eminem).songs.count', 1,
                          "Didn't assign 'Stan' to Eminem" do
          assert_difference 'Media::Song.count', 1,
                            "Didn't create 'Stan', and 'Stan' only" do
            e = Media::Event.create_by_hash(hash)
            assert e, "Didn't return event"
            assert_equal 1, e.artists.length, "Didn't assign one artist"
            assert_equal 'Eminem', e.artists[0].name,
                         "Didn't assign artist Eminem"
            assert_includes e.artists[0].songs.map(&:to_s), 'Not Afraid',
                            "Didn't assign song Not Afraid"
            assert_includes e.artists[0].songs.map(&:to_s), 'Stan',
                            "Didn't assign song Stan"
          end # songs
        end # eminem
      end # artists
    end # event
