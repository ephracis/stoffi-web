...
class SortableTest < ActiveSupport::TestCase
  ...
  test 'should sort songs' do
    ...
    assert_equal current_order, @album.songs.map(&:id),
                 "Wasn't in correct original order"

    # sort
    @album.sort :songs, new_order

    # verify new order
    assert_equal new_order, @album.ordered_songs.map(&:id),
                 "Didn't change to correct order"
  end
end
