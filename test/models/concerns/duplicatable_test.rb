...
class DuplicatableTest < ActiveSupport::TestCase
  ...
  test 'should combine habtm relation' do
    ...
    checked = []
    (song_a.genres + song_b.genres).each do |g|
      next if g.in? checked
      assert_includes song_b.genres, g,
                      "Genre #{g} not included in song #{song_b}"
      checked << g
    end
    song_b.genres.each do |g|
      assert_includes checked, genre,
                      "Genre #{g} not included in song #{song_b}"
    end
  ...
  test 'should fail to combine non-existing association' do
    assert_raise(ArgumentError) do
      Media::Song.include_associations_of_dups :listens,
                                               :this_relation_doesnt_exist,
                                               :shares
    end
  end
end
