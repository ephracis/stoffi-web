...
class Accounts::OauthControllerTest < ActionController::TestCase
  ...
  test "should skip authorization second time" do
    ...
    assert_includes user.get_apps(:added), app, "Test assumes user has added app"
    token = Accounts::RequestToken.create(
      token: randstr(special: false),
      app: app
    )
    callback = "#{app.callback_url}&oauth_token=#{token.token}&oauth_verifier="
    sign_in user
