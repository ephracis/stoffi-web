...
class Accounts::LinksControllerTest < ActionController::TestCase
  ...
  test "should refresh link for signed in user" do
    ...
    OAuth2::AccessToken.any_instance.expects(:post).at_least_once.returns(facebook_response)
    sign_in @user
    stub_for_settings
    ...
    assert_equal new_token, Accounts::Link.find(@link.id).access_token, "Didn't update token"
  end
  ...
  test "should create new for existing user" do
    ...
    assert_equal @user.id, warden.session_serializer.fetch(:user).id, "Didn't sign in"
  end
  ...
  test "should create new link and new user" do
    ...
    assert_equal User.last.id, warden.session_serializer.fetch(:user).id, "Didn't sign in"
  end
  ...
  test "should destroy link" do
    ...
    OAuth2::AccessToken.any_instance.expects(:get).times(0..99).
      with("/me/music.playlists?limit=25&offset=0").returns(facebook_response)
