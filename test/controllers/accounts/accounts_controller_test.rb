...
module Accounts
  class AccountsControllerTest < ActionController::TestCase
    ...
    test 'should update own account' do
      ...
      assert_equal 'new@mail.com', User.find(@user.id).email,
                   "Didn't change email"
    end
    ...
    test 'should not update other account' do
      ...
      assert_redirected_to edit_user_registration_path
      assert_not_equal 'new@mail.com', User.find(users(:charlie).id).email,
                       'Changed email'
    end
    ...
    test 'should update other account when admin' do
      ...
      post :update, id: users(:charlie), user: { email: 'new@mail.com' }
      assert_redirected_to edit_user_registration_path
      assert_equal 'new@mail.com', User.find(users(:charlie).id).email,
                   "Didn't change email"
    end
    ...
    test 'should delete profile' do
      ...
      assert_difference('Media::Playlist.count', @user.playlists.count * -1) do
      assert_difference('Media::Listen.count', @user.listens.count * -1) do
      assert_difference('Share.count', @user.shares.count * -1) do
      assert_difference('Accounts::Link.count', @user.links.count * -1) do
      assert_difference('Accounts::Device.count', @user.devices.count * -1) do
      assert_difference('App.count', @user.apps.count * -1) do
