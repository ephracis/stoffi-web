...
class Media::GenresControllerTest < ActionController::TestCase
  ...
  test "should not update genre logged out" do
    ...
    assert_redirected_to new_user_session_path
    assert_not_equal 'New name', Media::Genre.find(@genre.id).name, "Changed name"
  end
  ...
  test "should update genre as admin" do
    ...
    patch :update, id: @genre, genre: { name: 'New name' }
    assert_redirected_to genre_path(assigns(:genre))
    assert_equal 'New name', Media::Genre.find(@genre.id).name, "Didn't change name"
  end
