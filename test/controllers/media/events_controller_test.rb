...
module Media
  class EventsControllerTest < ActionController::TestCase
    ...
    test 'should not update logged out' do
      ...
      assert_redirected_to new_user_session_path
      assert_not_equal 'New name', Media::Event.find(@event.id).name,
                       'Changed name'
    end
    ...
    test 'should not update as user' do
      ...
      assert_redirected_to dashboard_path
      assert_not_equal new_name, Media::Event.find(@event.id).name,
                       'Changed name'
    end
    ...
    test 'should update as admin' do
      ...
      patch :update, id: @event, event: { name: new_name }
      assert_redirected_to event_path(assigns(:event))
      assert_equal new_name, Media::Event.find(@event.id).name,
                   "Didn't change name"
    end
