...
module Media
  class PlaylistsControllerTest < ActionController::TestCase
    ...
    test 'should not show non-existent' do
      get :show, user_slug: @playlist.user, playlist_slug: rand(1..10_000)
    ...
    test 'should not create two playlists with same name' do
      ...
      assert_no_difference('Media::Playlist.count') do
        post :create, user_slug: @user, songs: songs,
                      playlist: @playlist.attributes.slice('name',
                                                           'slug',
                                                           'is_public',
                                                           'filter')
      end
    ...
    test 'should unfollow playlist' do
      ...
      assert_redirected_to playlist_path(@playlist2),
                           'Could not follow playlist'
      assert @user.follows?(@playlist2), 'Not following playlist'
      delete :destroy, playlist_slug: @playlist2.to_param,
                       user_slug: @playlist2.user
      assert_redirected_to playlist_path(@playlist2),
                           'Could not unfollow playlist'
