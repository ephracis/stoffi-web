...
class StaticControllerTest < ActionController::TestCase
  ...
  test 'should send contact mail' do
    post :mail, name: 'Alice', email: 'alice@mail.com', subject: 'A Title',
                message: 'my message is not very short'
  ...
  test 'should get old browser warning' do
    [
      ...
      'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_9) AppleWebKit/400.71 '\
      '(KHTML, like Gecko)'
    ].each do |ua|
      cookies.delete(:skip_old)
      @request.headers['HTTP_USER_AGENT'] = ua
  ...
  test 'should get pass old browser warning' do
    [
      ...
      get :about
      assert_response :success
      assert_select 'div#old', false,
                    "Could not get pass old browser warning on #{ua}"
    end
  end
end
