...
module Backends
  module SoundcloudTestHelpers
    ...
    def stub_soundcloud_search(klass, resources)
      ...
      stub_request(:any, %r{https://api.soundcloud.com/tracks\.json.*q=.*})
        .to_return(body: resources.to_json, status: 200)
    end

    # Stub a request for a specific song.
    #
    # `song` is a hash describing the song. A random value is used unless the
    # value is specified.
    def stub_soundcloud_song(song)
      song = soundcloud('song').deep_merge song.stringify_keys
      stub_request(:any, %r{https://api.soundcloud.com/tracks/\w+\.json.*})
