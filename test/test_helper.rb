...
class ActiveSupport::TestCase
  ...
  def randstr(options = {})
    default = options.all? { |k,v| v == false }
    options = {
      length: 20, lower: default, upper: default, special: default,
      numbers: default
    }.merge(options)
    chars = ''
    chars += 'abcdefghjklmnopqrstuvwxyz' if options[:lower]
    chars += 'ABCDEFGHJKLMNOPQRSTUVWXYZ' if options[:upper]
    chars += '0123456789' if options[:numbers]
    # special are multiplied to increase their probability
    chars += "`!@#$%^&*()_+{}|:\\\"\'~-=[];',./?<>"*5 if options[:special]
